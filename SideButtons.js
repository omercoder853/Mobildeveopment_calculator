...
export default function SideButtons({ sideButtons, width, height, handlePress, isHorizontal }) {
    let btn_height;
    let btn_width;
    if (isHorizontal) {
        ...
        <View style={styles.side_area}>
            {sideButtons.map((btn, ind) => (
                <TouchableOpacity key={ind} onPress={() => handlePress(btn.label, btn.action)}
                    style={[styles.default_button, { width: btn_width, height: btn.is_double ? btn_height * 2 + margin_btm : btn_height, marginBottom: margin_btm }
                    ]}>
